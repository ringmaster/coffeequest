...
This guide covers best practices for creating quests in Coffee Quest. Quests are JSON files containing step definitions that the game engine selects based on player location and tags. Good quest design creates meaningful choices, rewards player effort proportionally, and avoids dead ends.

## JSON Schema Reference

### Quest File Structure

A quest file is a JSON object with a single `steps` array:
...
{
  "id": "Tavern",
  "tags": ["!quest", "!done:tavern_brawl"],
  "vars": {
    "drunk": ["Olaf", "Willem", "Bertram"]
  },
...
```json
"vars": {
  "npc_name": ["Alice", "Bob", "Carol"],
  "mood": ["angry", "sad", "nervous"]
}
```
...
"vars": {
  "victim": [
    {"name": "Elena", "occupation": "merchant", "gender": "female"},
    {"name": "Marcus", "occupation": "trader", "gender": "male"}
  ]
}
```
...
{
  "id": "complete_step",
  "tags": ["-quest", "+done:quest_name", "+inv:silver"],
  "text": "Quest complete!",
...
{
  "id": "Town Square",
  "tags": ["@q:has_flower", "@q:talked_to_herbalist", "q:fetch_quest"],
...
{
  "id": "Town Square", 
  "tags": ["@q:has_flower", "q:fetch_quest"],
...
1. **Before quest accepted** — Default location behavior (handled by `default-locations.json`)
...
{
  "id": "Guardhouse",
  "tags": ["@q:lead_blacksmith", "q:armory"],
...
{
  "id": "give_flower",
  "tags": ["-inv:moonflower", "+q:flower_delivered"],
...
{
  "id": "Church",
  "tags": ["q:fetch_flower", "!inv:moonflower"],
  "text": "The cleric looks up hopefully, then sighs when she sees your empty hands. \"The moonflower only blooms in the deep forest, far from town. Did you search thoroughly?\""
}
```
...
{
  "id": "Church",
  "tags": ["q:fetch_flower", "!inv:moonflower", "!q:hint_given"],
...
```json
{
  ...
  "vars": {
    "tavern_mood": [
      "The tavern is empty except for the barkeep, who is cleaning up broken chairs from last night's debauchery.",
...
```json
{
  "text": "The {{victim.occupation}} shows you {{victim.his}} scar. {{victim.He}} seems to be who {{victim.he}} says {{victim.he}} is."
}
```
...
```json
{
  "id": "accept_quest",
  "vars": {
    "guilty": ["guard", "smith", "madam"]
  },
  "tags": ["+quest", "+q:mystery", "+q:guilty_{{guilty}}"],
...
{
  "id": "find_evidence",
  "tags": ["q:mystery", "@q:guilty_guard"],
  "text": "Evidence pointing to the guard..."
}
```
...
{
  "id": "Tavern",
  "tags": ["!quest", "!done:epic_heist", "@ally:thieves_guild", "@done:armory"],
  "text": "A hooded figure approaches. \"Word is you helped the Guard... and you know people who work outside the law. I have a job that needs both kinds of expertise.\""
}
```

This quest requires:
- `ally:thieves_guild` — earned from a prior quest
- `done:armory` — completed the Armory Audit
...
Players who earned `ally:guard` from the Armory Audit see a hidden option that bypasses the challenge entirely. Others must pay or sneak—but the quest remains completable either way.
...
|------|---------------|---------------------|
| `inv:armory_key` | Armory Audit | Access restricted areas |
...
```json
{
  ...
  "tags": ["-quest", "+done:armory", "-inv:armory_key", "+inv:silver"],
...
{
  "id": "complete_armory_quest",
  "tags": ["-quest", "+done:armory", "+ally:guard"],
...
Create log entries when the player **takes an action**, not when they merely arrive somewhere:
...
| `inv:armory_key` | Armory Audit | Access to Armory (if not returned) |

### Reputation Tags

| Tag | Source | Potential Uses |
|-----|--------|----------------|
| `ally:guard` | Armory Audit (best ending) | Bypass guard challenges, get information |
